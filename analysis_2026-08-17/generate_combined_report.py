...
============================
Combined variant of ../04_secondlevel_group/dumpFirstLevel_betaCoefs/generate_report.py
that reports BOTH first-level models side by side:
  - the amplitude-modulated model (corsi_easy/corsi_hard, corsi-score-weighted)
  - the na (non-amplitude-modulated) model (b1-b4, static amplitude)

Each COMPARISONS entry now carries its own base_dir (the two models' results
live under different top-level folders), and a `model` label used to group
and interleave the two models' sections per comparison.

Reads all 12 comparison result CSVs (6 per model, produced by
analyze_psd_agg_vs_psd_nagg.py) and generates two report files:

  • report.md   — Markdown (readable in VS Code / GitHub)
  • report.html — Styled HTML (open in browser)

Usage (from analysis_2026-08-17/):
  python generate_combined_report.py [--output-dir .] [--alpha 0.05]
"""
...
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # afni_fnirs/
_OLD_BASE = os.path.join(_ROOT, "04_secondlevel_group", "dumpFirstLevel_betaCoefs", "analysis_2026-08-17")
_NA_BASE  = os.path.join(_ROOT, "04p5_secondlevel_group_na", "dumpFirstLevel_betaCoefs", "analysis_2026-08-17")

# sans/saps excluded from all covariate sets in the 2026-08-17 rerun (both
# models) - see ../notes.txt
COVARS_HC  = "WAIS-matrix, SUD"
COVARS_PSD = "WAIS-matrix, SUD, DOSE"

# ──────────────────────────────────────────────────────────────────────────────
# Comparison manifest — one pair (amplitude-modulated + na) per group-pair x
# contrast, interleaved so the two models sit next to each other in the report.
# ──────────────────────────────────────────────────────────────────────────────
COMPARISONS = [
    dict(model="Amplitude-modulated", label="PSD+AGG vs HC", contrast="hard-easy",
         group_pos="PSD+AGG", group_neg="HC", covariates=COVARS_HC,
         base_dir=_OLD_BASE, result_dir="results_2026-08-17_psd_agg_vs_hc_hard_easy"),
    dict(model="na (non-amp-mod)", label="PSD+AGG vs HC", contrast="hard-easy",
         group_pos="PSD+AGG", group_neg="HC", covariates=COVARS_HC,
         base_dir=_NA_BASE, result_dir="results_psd_agg_vs_hc_na_hard_easy"),

    dict(model="Amplitude-modulated", label="PSD+AGG vs HC", contrast="corsi",
         group_pos="PSD+AGG", group_neg="HC", covariates=COVARS_HC,
         base_dir=_OLD_BASE, result_dir="results_2026-08-17_psd_agg_vs_hc_corsi"),
    dict(model="na (non-amp-mod)", label="PSD+AGG vs HC", contrast="corsi",
         group_pos="PSD+AGG", group_neg="HC", covariates=COVARS_HC,
         base_dir=_NA_BASE, result_dir="results_psd_agg_vs_hc_na_corsi"),

    dict(model="Amplitude-modulated", label="PSD+AGG vs PSD-AGG", contrast="hard-easy",
         group_pos="PSD+AGG", group_neg="PSD-AGG", covariates=COVARS_PSD,
         base_dir=_OLD_BASE, result_dir="results_2026-08-17_psd_agg_vs_psd_nagg_hard_easy"),
    dict(model="na (non-amp-mod)", label="PSD+AGG vs PSD-AGG", contrast="hard-easy",
         group_pos="PSD+AGG", group_neg="PSD-AGG", covariates=COVARS_PSD,
         base_dir=_NA_BASE, result_dir="results_psd_agg_vs_psd_nagg_na_hard_easy"),

    dict(model="Amplitude-modulated", label="PSD+AGG vs PSD-AGG", contrast="corsi",
         group_pos="PSD+AGG", group_neg="PSD-AGG", covariates=COVARS_PSD,
         base_dir=_OLD_BASE, result_dir="results_2026-08-17_psd_agg_vs_psd_nagg_corsi"),
    dict(model="na (non-amp-mod)", label="PSD+AGG vs PSD-AGG", contrast="corsi",
         group_pos="PSD+AGG", group_neg="PSD-AGG", covariates=COVARS_PSD,
         base_dir=_NA_BASE, result_dir="results_psd_agg_vs_psd_nagg_na_corsi"),

    dict(model="Amplitude-modulated", label="PSD-AGG vs HC", contrast="hard-easy",
         group_pos="PSD-AGG", group_neg="HC", covariates=COVARS_HC,
         base_dir=_OLD_BASE, result_dir="results_2026-08-17_psd_nagg_vs_hc_hard_easy"),
    dict(model="na (non-amp-mod)", label="PSD-AGG vs HC", contrast="hard-easy",
         group_pos="PSD-AGG", group_neg="HC", covariates=COVARS_HC,
         base_dir=_NA_BASE, result_dir="results_psd_nagg_vs_hc_na_hard_easy"),

    dict(model="Amplitude-modulated", label="PSD-AGG vs HC", contrast="corsi",
         group_pos="PSD-AGG", group_neg="HC", covariates=COVARS_HC,
         base_dir=_OLD_BASE, result_dir="results_2026-08-17_psd_nagg_vs_hc_corsi"),
    dict(model="na (non-amp-mod)", label="PSD-AGG vs HC", contrast="corsi",
         group_pos="PSD-AGG", group_neg="HC", covariates=COVARS_HC,
         base_dir=_NA_BASE, result_dir="results_psd_nagg_vs_hc_na_corsi"),
]
...
def load(comp, label_map):
    name = os.path.basename(comp["result_dir"].rstrip("/"))
    path = os.path.join(comp["base_dir"], comp["result_dir"], f"{name}.csv")
...
def load_interactions(comp, label_map):
    name = os.path.basename(comp["result_dir"].rstrip("/"))
    path = os.path.join(comp["base_dir"], comp["result_dir"], f"{name}_interactions.csv")
    if not os.path.exists(path):
        return None
    df = pd.read_csv(path)
    df["ch_label"] = _relabel(df["channel"], label_map)
    ...
    for comp, df, clusters in all_data:
        if clusters:
            for cl in clusters:
                ...
                lines.append(
                    f"- **[{comp['model']}] {comp['label']}** ({comp['contrast']}): cluster [{chs}], "
                    f"p={cl['p']:.3f}, mass={cl['mass']:.2f} {cl['direction']}"
                )
    if not lines:
        return "No cluster-corrected significant findings were observed across any of the twelve comparisons."

    intro = (
        "Across the twelve comparisons (both models), cluster-based permutation correction (FWER) identified "
...
# Markdown builder
# ──────────────────────────────────────────────────────────────────────────────
def build_markdown(all_data, alpha, label_map, mode="channels"):
    today = datetime.date.today().isoformat()
    L = []

    L += [
        "# fNIRS Second-Level Group Analysis — Combined Report (Amplitude-Modulated + na)",
        "",
        f"**Generated:** {today}  ",
        "**Models:** Amplitude-modulated (corsi_easy/corsi_hard, corsi-score-weighted) "
        "and na (b1-b4, static amplitude) — see analysis_report_2026-08-17 for what "
        "changed and why both were rerun  ",
        f"**Alpha:** {alpha}  ",
    ...
    if mode == "regions":
        ...
        L += [
            "## Overview of Significant Findings",
            "",
            "| # | Model | Comparison | Contrast | Sig. cluster (p) | Perm max-stat chs | FDR-BH chs |",
            "|---|---|---|---|---|---|---|",
        ]
        for i, (comp, df, clusters) in enumerate(all_data, 1):
            clust_str = " ; ".join(
            ...
            fdr  = sig_channels(df, "ancova_p_fdr_sig")
            L.append(
                f"| {i} | {comp['model']} | **{comp['label']}** | {comp['contrast']} "
    ...
    for i, (comp, df, clusters) in enumerate(all_data, 1):
        ...
        int_result = load_interactions(comp, label_map)
...
def build_html(all_data, alpha, label_map, mode="channels"):
    today = datetime.date.today().isoformat()
    B = []
    ...
    B += [
        f"<h1>fNIRS Second-Level Group Analysis &mdash; Combined Report (Amplitude-Modulated + na)</h1>",
        f"<p><b>Generated:</b> {today} &nbsp;|&nbsp; <b>Alpha:</b> {alpha} &nbsp;|&nbsp; "
        f"<b>Permutations:</b> 5,000</p>",
        "<p><b>Models:</b> Amplitude-modulated (corsi_easy/corsi_hard, corsi-score-weighted) "
        "and na (b1&ndash;b4, static amplitude) &mdash; see "
        "<code>analysis_report_2026-08-17</code> for what changed and why both were rerun</p>",
    ...
    if mode == "regions":
        ...
        B += [
            "<h2>Overview of Significant Findings</h2>",
            "<table><tr><th>#</th><th>Model</th><th>Comparison</th><th>Contrast</th>"
        ...
        for i, (comp, df, clusters) in enumerate(all_data, 1):
            ...
            fdr  = sig_channels(df, "ancova_p_fdr_sig")
            B.append(
                f"<tr><td>{i}</td><td>{comp['model']}</td><td><b>{comp['label']}</b></td><td>{comp['contrast']}</td>"
                f"<td>{clust_td}</td>"
                f"<td>{'<b>' + ', '.join(pmax) + '</b>' if pmax else '<span class=\"none\">—</span>'}</td>"
                f"<td>{', '.join(fdr) if fdr else '<span class=\"none\">—</span>'}</td></tr>"
            )
    ...
    for i, (comp, df, clusters) in enumerate(all_data, 1):
        B += [
            f"<h2>{i}. [{comp['model']}] {comp['label']} &mdash; {comp['contrast']}</h2>",
    ...
    B.append(
        f"<footer>Report generated by <code>generate_combined_report.py</code> on {today}. "
    ...
    return (
        ...
        f'<title>fNIRS Combined Report {today}</title>'
...
def main():
    ...
    label_map = TARGET_LABEL_MAP_CHANNELS
    stem = "report"

    # Load all comparisons
    all_data = []
    for comp in COMPARISONS:
        df = load(comp, label_map)
        if df is None:
            print(f"  [WARN] Missing results for: {comp['model']} / {comp['result_dir']}")
            continue
        clusters = extract_clusters(df)
        all_data.append((comp, df, clusters))

    print(f"Loaded {len(all_data)}/{len(COMPARISONS)} comparisons.")

    md   = build_markdown(all_data, args.alpha, label_map)
    html = build_html(all_data, args.alpha, label_map)
