#!/usr/bin/env python3
"""
prep_select_cov_local.py
======================
Fixed-path variant of ./prep_select_cov.py.
...
xlsx). prep_select_cov.py itself is also hardcoded to a /mnt/speyside mount
that doesn't exist here; this variant uses local /mnt/highlands paths and
writes a local copy (select.cov.local.1D) instead of overwriting the
...
Output: select.cov.local.1D, columns X sans saps wais_matrix sud dose
"""
...
OUT_1D    = Path(__file__).parent / "select.cov.local.1D"
